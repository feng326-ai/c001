...
> **状态**：v2.0-draft（2026-08-22），是新开发唯一的**架构边界**。核心领域的结构级字段、幂等键和状态骨架已由 `docs/核心领域契约_v1.md` 冻结；完整 OpenAPI/事件 JSON Schema、原因码和错误码仍需冻结并升为 `v2.0` 后，各窗口才可并行实现业务接口。旧 v1 的 Redis/Celery 与只读 OA 方案仅用于现有节点过渡，不得复制到新系统。
