...
| `COLLECT-STABILITY-001` | 集成负责人 `/root`（采集侧只读审查：协作智能体） | 共享工作区 / `main` | `wxsearch/unattended.py`<br>`wxsearch/config.py`<br>`wxsearch/tasks.py`<br>`wxsearch/task_scheduler.py`<br>`tests/test_unattended_heartbeat.py`<br>`tests/test_device_leases.py`<br>`tests/test_device_leases_integration.py`<br>`docs/migrations/008_keyword_channel_state.sql`<br>`docs/决策记录.md`<br>`docs/坑位手册.md`<br>`docs/开发批次/ACTIVE.md` | D-022、v2 `(channel,keyword,lease_id)`；旧 VM 调用向后兼容；迁移 008 + 渠道完整播种硬前置 | 进行中 | 32 项快速门禁通过，10 项真实 PG/Redis 用例待一次性 QA；现网 `souyisou` 63 行且缺失数为 0 已只读核验，随后做单 VM canary，跨过长关键词且无误释放后再决定扩量 |
...
- 当前只有 `COLLECT-STABILITY-001` 获得采集循环代码租约；没有数据库迁移或生产部署租约。
