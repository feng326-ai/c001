...
协议：宿主为每台节点生成 `control-<device_id>.json`，其中 `payload_b64` 是 UTF-8 JSON，`signature_b64` 是该原始 payload 的 HMAC-SHA256。payload 必含 `device_id`、`sequence`、`issued_at`、`expires_at`、`action`、`args`。agent 通过 `control_url/control-<device_id>.json` 拉取，拒绝签名错误、身份不匹配、过期和重放的消息。
